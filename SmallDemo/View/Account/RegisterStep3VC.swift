  ...
  class RegisterStep3VC: GlobalVC {
    ...
    override func ButtonTap(tag: Int) {
        ...
        if(txtPwd.text!.trim().length>=6 && txtPwd.text ==  txtConfrimPwd.text!.trim())
        {
            ...
            if(!isFromFindPwd)
            {
                ...
                self.httpPostApi(AppConfig.ConstUrl.Register, body: parameters, tag: 11)
            }
            else
            {
                let parameters = [
                ...
                self.httpPostApi(AppConfig.ConstUrl.ResetPwd, body: parameters, tag: 12)
            }
        }
        else
        {
       ...
       //     AppConfig.sharedAppConfig.MySex = jsonData["MySex"].intValue
        ...
        }else if (tag == 12)
        {
            ...
            if (result == 1)
            {
                ...
                let vc:LoginVC = UIHelper.GetVCWithIDFromStoryBoard(.Account, viewControllerIdentity: "LoginVC") as! LoginVC
