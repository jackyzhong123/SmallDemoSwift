...
class RegisterStep2VC: GlobalVC {
    ...
    func changeResendTime()
    {
        ...
        {
            secondRemainResend--;
            let msg = String(secondRemainResend) + "秒后重新获取"
    ...
    override func ButtonTap(tag: Int) {
        ...
        if(tag == 10)//重新发送验证码
        {
            ...
            let OKAction = UIAlertAction(title: "确认", style: .Default) { (action) in
                ...
                self.httpGetApi(AppConfig.ConstUrl.SendSMS, body:parameters, tag: 99)
        ...
        else if(tag == 11)//提交验证码
        {
            if(txtVerifyCode.text!.trim().length>0)
            {
                ...
                if(!isFromChangMobile)
                {
                    ...
                    self.httpGetApi(AppConfig.ConstUrl.SMSVerify, body: parameters, tag: 11)
                    return
                }else{
                    parameters = [
                        "mp": self.txtPhone.text!,
    ...
    override func requestDataComplete(response:AnyObject,tag:Int)
    {
        ...
        if (tag == 11) {
            ...
            if(!isFromChangMobile)
            {
                ...
                let vc:RegisterStep3VC = UIHelper.GetVCWithIDFromStoryBoard(.Account, viewControllerIdentity: "RegisterStep3VC") as! RegisterStep3VC
